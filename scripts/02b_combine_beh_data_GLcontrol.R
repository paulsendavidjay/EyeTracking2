# THIS SCRIPT PROCESSES DATA FOR CONTROLS ONLY
...
for (subj in c(90,91,92,93,94,95)) {
	#preprocess(subj)
 	fileName <- paste(c(processed_data_dir, "beh_data_", subj, ".R"), collapse="" )
 	load(fileName)

 	if (!is.null(beh_data$Pause)) {
 		beh_data$Pause <- NULL
 		beh_data$endPause <- NULL
 	}
...
cmbd_outcome_pupil_file_name <- paste(c(study_dir, "/data/processed_data/pupil_data_cmbd_outcome_pupilcontrol.R"), collapse="")
unlink(cmbd_outcome_pupil_file_name)
save(cmbd_outcome_pupil, file= cmbd_outcome_pupil_file_name)

cmbd_riskData_file_name <- paste(c(study_dir, "/data/processed_data/beh_data_cmbd_riskDatacontrol.R"), collapse="")
