...
#								(ALREADY PERFORMED IN EARLIER VERSION)
...
# for (domain in c("Gain","Loss")) {
	...
	# if (sum(current_model_df$convergence > 0) > 0) {
		...
		# print(current_convergence[,c("subjectID","convergence")])
	# } else {
		# cat(paste(c("\nmodel=", model, "\ndecay_type=", decay_type, "\ndomain=\"", domain, "\"\nGOOD\n"),sep="",collapse=""))
	# }
...
out_file_data_name <- "../analysis/_analysis_emot/model_comparisons_AIC_z_childs_only.R"
...
#dev.new(height=4,width=4)
pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_AIC_model_comparisons_child.pdf",height=4, width=4)
ggplot(aic_z_excld_nonconverg_df, aes(x=model, y= AIC_adjusted_excld)) +
	...
	labs(title="Model Comparison: Children", x="", y="Adjusted AIC", fill="N Decay\nParams") +
...
#dev.new(height=4,width=9)
pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Gain_all.pdf",height=4,width=9)
ggplot(subset(aic_z_excld_nonconverg_df_long, domain=="Gain" & ! variable %in% uninteresting_vars & decay_type != 0), aes(x= variable, y=value, fill= decay_type)) +
	...
	theme(axis.text.x = element_text(size=6),
		...
		strip.text.x = element_text(size=9),
		title = element_text(size=12)) +
	labs(title="Model Parameters Child: Gain Domain", x="", y="z value", fill="N Decay\nParams") +
...
#dev.new(height=4,width=9)
pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Gain_2-4.pdf",height=4,width=6)
...
#dev.new(height=4,width=9)
pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Loss_all.pdf",height=4,width=9)
ggplot(subset(aic_z_excld_nonconverg_df_long, domain=="Loss" & ! variable %in% uninteresting_vars & decay_type != 0), aes(x= variable, y=value, fill= decay_type)) +
	...
	theme(axis.text.x = element_text(size=6),
		...
		strip.text.x = element_text(size=9),
		title = element_text(size=12)) +
	labs(title="Model Parameters Child: Loss Domain", x="", y="z value", fill="N Decay\nParams") +
	scale_fill_manual(values=cols[2:4], breaks=c("1","2","3"), labels=c("1","2","4")) 
dev.off()
...
#dev.new(height=4,width=9)
pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Loss_2-4.pdf",height=4,width=9)
...
#dev.new(height=4,width=9)
pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Gain_vs_Loss_model2_.pdf",height=4,width=6)

ggplot(subset(aic_z_excld_nonconverg_df_long, model == "Regret & Relief X\nAction & Inaction\nDissapointment, Elation" & !variable %in% uninteresting_vars & decay_type == 3 & !variable %in% c("disappointment", "elation")), aes(x= variable, y=value, fill=domain)) +
	geom_bar(stat="identity", position="dodge") +
	...
	theme(axis.text.x = element_text(size=6),
		...
		strip.text.x = element_text(size=9),
		title = element_text(size=12)) +
	labs(title="Gain & Loss Domains Child", x="", y="z value")
dev.off()


pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Loss_model3_.pdf",height=4,width=3.5)
ggplot(subset(aic_z_excld_nonconverg_df_long, domain == "Loss" & model == "Regret & Relief X\nAction & Inaction\nDissapointment, Elation" & !variable %in% uninteresting_vars & decay_type == 3), aes(x= variable, y=value, fill=domain)) +
	...
	theme(axis.text.x = element_text(size=6),
		...
		strip.text.x = element_text(size=9),
		title = element_text(size=12)) +
	labs(title="Child", x="", y="z value")
dev.off()

pdf("~/Documents/Academics/Projects/EyeTrackingGaze/analysis/_analysis_emot/_child_params_Gain_model3_.pdf",height=4,width=3.5)
ggplot(subset(aic_z_excld_nonconverg_df_long, domain == "Gain" & model == "Regret & Relief X\nAction & Inaction\nDissapointment, Elation" & !variable %in% uninteresting_vars & decay_type == 3), aes(x= variable, y=value, fill=domain)) +
