plot_overlay_gaze_GL_example <- function(eye_data, subjectID, trial_num, epoch, opt_title="") {
...
#	source(paste(c(study_dir, '/scripts/functions/plotGaze.R'   ), collapse=""))
	...
	current_condition <- eye_data.current$condition[1] 
	...
	gradient.pos <- c(1145, 288, 1165, 88) # x_left, y_bottom, x_right, y_top
...
################################################################################################
	...
	text(gradient.pos[1], gradient.pos[4], startTime, adj=c(0.5,-0.25), col="white", cex=.8) # first trialTime
	text(gradient.pos[1], gradient.pos[2], endTime, adj=c(0.5,1.25), col="white", cex=.8) # last trialTime
