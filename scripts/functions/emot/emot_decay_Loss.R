...
emot_decay_Loss <- function(decay_temp, trial, choice, outcome, mag1.prop, mag2.prop, sure2.prop, output_type, ADJUST_EMOT_AT_SURE_BET) {
	...
	for (i in 1:n_obs) {
		...
		if (outcome[i] == 2) { # LOSS FRAME LOSS
			...
			if (ADJUST_EMOT_AT_SURE_BET) {
				############# 	BEGIN SURE ADJUST
				# ADJUST POSITIVE EMOTION
				relief[i] <- mag1.prop[i] # LARGE MAG
				# ADJUST NEGATIVE EMOTION
				regret[i] <- mag2.prop[i] # SMALL MAG
				############# 	END SURE ADJUST
