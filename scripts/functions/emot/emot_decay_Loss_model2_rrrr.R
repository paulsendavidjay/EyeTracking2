...
emot_decay_Loss_model2_rrrr <- function(decay_temp, trial, choice, outcome, mag1.prop, mag2.prop, sure2.prop, output_type, decay_type) {
	...
	for (i in 1:n_obs) {
		...
		if (outcome[i] == 0) { #WINS
			...
			# ADJUST POSITIVE EMOTION
			relief_inaction[i] <- mag1.prop[i] # WINS: sure time [counterfactual choice]			

			# ADJUST NEGATIVE EMOTION
			regret_inaction[i] <- mag2.prop[i] # LOSSES: sure time [counterfactual choice]
	...
	relief_inaction <- relief_inaction0
		...
		glm1 <- glm(choice ~ regret + relief + regret_inaction + relief_inaction, family=binomial(link = "logit"))


		if (output_type == "LL") {
			-1*logLik(glm1)[1]
		} else if (output_type == "data") {
			cbind(regret, relief, regret_inaction, relief_inaction, prev_outcome)
		}
