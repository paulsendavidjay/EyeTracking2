...
emot_decay_Gain_model1_rr <- function(decay_temp, trial, choice, outcome, mag1.prop, mag2.prop, sure2.prop, output_type, decay_type) {
	...
	for (i in 1:n_obs) {
		...
		if (outcome[i] == 2) { #WINS
			...
			# ADJUST POSITIVE EMOTION
			relief[i] <- -1*mag2.prop[i] # WINS: sure time [counterfactual choice]			

			# ADJUST NEGATIVE EMOTION
			regret[i] <- -1*mag1.prop[i] # LOSSES: sure time [counterfactual choice]
