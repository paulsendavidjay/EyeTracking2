...
emot_decay_Gain <- function(decay_temp, choice, outcome, mag1.prop, mag2.prop, sure2.prop, output_type, ADJUST_EMOT_AT_SURE_BET) {
	...
	for (i in 1:n_obs) {
		...
		if (outcome[i] == 2) { #WINS
			...
			if (ADJUST_EMOT_AT_SURE_BET) {
				############# 	BEGIN SURE ADJUST
				# ADJUST POSITIVE EMOTION
				relief[i] <- mag2.prop[i] # WINS: sure time [counterfactual choice]			
				# ADJUST NEGATIVE EMOTION
				regret[i] <- mag1.prop[i] # LOSSES: sure time [counterfactual choice]
				############# 	END SURE ADJUST
