...
emot_decay_Loss <- function(decay_temp, choice, outcome, mag1.prop, mag2.prop, sure2.prop, output_type, ADJUST_EMOT_AT_SURE_BET) {
	
	ADJUST_EMOT_AT_SURE_BET = ADJUST_EMOT_AT_SURE_BET
	...
	for (i in 1:n_obs) {
		...
		if (outcome[i] == 2) { # LOSS FRAME LOSS


			if (ADJUST_EMOT_AT_SURE_BET) { # if we adjust regret and relief at sure bet, it's sure bet only we adjust
		...
		} else if (outcome[i] == 0) { # LOSS FRAME WIN
			...
			if (ADJUST_EMOT_AT_SURE_BET) { # if we adjust regret and relief at sure bet, it's sure bet only we adjust
				...
				elation[i] <- mag1.prop[i] # WINS: loss time [counterfactual outcome]
			...
			if (ADJUST_EMOT_AT_SURE_BET) {
				############# 	BEGIN SURE ADJUST
				# ADJUST POSITIVE EMOTION
				relief[i] <- mag1.prop[i] # WINS: sure time [counterfactual choice]			
				# ADJUST NEGATIVE EMOTION
				regret[i] <- mag2.prop[i] # LOSSES: sure time [counterfactual choice]
				############# 	END SURE ADJUST
				...
				if ( i > 1) {
					...
					############# 	BEGIN SURE ADJUST			
					# UPDATE POSITIVE EMOTION
					relief[i] <- relief[i] + pos_decay*relief[i-1] # WINS: loss time [counterfactual choice & outcome]
					# UPDATE NEGATIVE EMOTION
					regret[i] <- regret[i] + neg_decay*regret[i-1] # LOSSES: sure time [counterfactual choice & outcome]
					############# 	END SURE ADJUST
				}
	
			} else	{ # if not ADJUST_EMOT_AT_SURE_BET
			...
			} # if/else ADJUST_EMOT_AT_SURE_BET
	...
	if (ADJUST_EMOT_AT_SURE_BET) { # if we adjust regret and relief at sure bet, it's sure bet only we adjust
		...
		if (output_type == "LL") {
			...
			cbind(disappointment, regret, loss, elation, relief, gain, prev_outcome)
		}
