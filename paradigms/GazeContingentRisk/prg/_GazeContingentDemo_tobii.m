function GazeContingentDemo_tobii(mode, ms, myimgfile,hostName, port)
% 
% 11/12/07  Modifiend by  Fani Deligianni  - Use talk2tobii
% GazeContingentDemo has been modified to allow using it with talk2tobii
% toolbox. Hostname is the IP address of the machine that runs the TET
% server. Port is the port of the host machine that allows to connect to
% the TET server.
...
try
    ...
    % Initialize Tobii eye position and calibration
    if(EYETRACKER)
        res = wRect(3:4);
        errorCode = TobiiInit(hostName, port, w, res);
        if( errorCode )
            EYETRACKER = 0;
        end
    end
        ...
        if (mx~=mxold | my~=myold)            
            ...
            if ~IsEmptyRect(dRect)
                ...
                if( EYETRACKER )
                    ...
                    CursorEvent = {'CursorEvent',mx,my};
                    
                    EventList{count} = {EyeEvent, FlipEvent, CursorEvent};
                    count = count+1; 
                end
    ...
    if( EYETRACKER )
        ...
        for i=1:length(EventList)
            ...
            FlipEvent = EventList{i}{2};
            CursorEvent = EventList{i}{3};
            
            talk2tobii('EVENT','FLIP',FlipEvent{2},0,...
                            'EyeEventSec',EyeEvent{2},...
                            'EyeEventMsec',EyeEvent{3},...
                            'CursorX', CursorEvent{2},...
                            'CursorY', CursorEvent{3} );
        end
        outPicDir = './';
