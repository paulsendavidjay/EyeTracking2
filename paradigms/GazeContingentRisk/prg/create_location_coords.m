function [ stim_size, location_coords ] = create_location_coords( nrows, ncols, window_dim, stim_dim, h_margin, v_margin, h_gap, v_gap)
...
%{
    ...
    h_gap, v_gap = horizontal, vertical gaps in percent(decimal) of stimulus width, height
%}

stimulus_ht = stim_dim(1);
stimulus_wd = stim_dim(2);
h_gap = h_gap*stimulus_wd;
v_gap = v_gap*stimulus_ht;
...
total_stimulus_width = ncols*stimulus_wd + (ncols-1)*h_gap;
...
% specifications
stim_width_offset = ( (stimulus_window_dim(3) - stimulus_window_dim(1)) - total_stimulus_width)/2;
stim_height_offset = ( (stimulus_window_dim(4) - stimulus_window_dim(2)) - total_stimulus_height)/2;
...
for y = 1:nrows
    for x = 1:ncols
        x1(stim_position) = floor(x1_(x));
        x2(stim_position) = floor(x2_(x));
        y1(stim_position) = floor(y1_(y));
        y2(stim_position) = floor(y2_(y));
