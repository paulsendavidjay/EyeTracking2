...
trialList = 'trials_list_Frontiers1GL_practice.txt';
Param = importdata(trialList, '\t', 1);
ntrials_per_block = 6; % used for dislpaying winnings and for time bar
...
if (~ DEBUG )
    ...
    fprintf(fid1,'trial trialType sure1 sure2 mag1 mag2 choice gamble coinsWon P1_image P1 P2_image P2 vert topRight sideMag sideSure fixTime presTime decisionTime respTime confirm_delayTime outcomeTime RT choiceCount sure1_fix sure2_fix mag1_fix mag2_fix\n');
...
%         
  ...
  %{  
    ...
    if (mod(trial, ntrials_per_block) == 0) %
